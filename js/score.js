...
function computeQualityScore(){
  ...
  push(!!state.shortName.trim() && state.shortName.length <= 12, 10, 'Short name fits the 12-character home screen limit');
  push(!!state.description.trim(), 8, 'Description is filled in (helps install prompts and app stores)');
  ...
  push(!!state.iconImg && state.iconImg.width >= 512 && state.iconImg.height >= 512, 10, 'Icon is 512\u00d7512 or larger (required for maskable + splash quality)');
  push(!!state.iconImg && state.iconImg.width === state.iconImg.height, 6, 'Icon is square (no cropping needed)');
...
function scoreRingSvg(total){
  ...
  const color = total >= 85 ? 'var(--success)' : total >= 60 ? 'var(--warn)' : 'var(--danger)';
  return `
